...
public class IMItemTypeEditFlow extends GeneralActions {
	WebDriver driver;
	Truelocatelogin truelocatelogin;
	IMItemTypeElements imttemtypeelements ;
	...
	public void setUp() throws IOException {
		...
		imttemtypeelements = PageFactory.initElements(driver, IMItemTypeElements.class);
	...
	public void itemTypeEditFlow(String sValue,String sCode,String sName,String sDescription)throws Exception {
		try {
			imttemtypeelements.menuClick();
			Thread.sleep(1000);
			ReusableActions.takeSnapshot(driver, className);
			...
			imttemtypeelements.enterItemTypeInfo(sCode, sName, sDescription);
			...
			if (alertMessage.equalsIgnoreCase(constants.edit_itemtype_successmsg)) {
				TestNGResults.put("9", new Object[] { "ItemType edit screen",
						"ItemType edited successfully", "Pass" });	
				Assert.assertEquals(alertMessage, constants.edit_itemtype_successmsg);
			} else {
				TestNGResults.put("9", new Object[] { "ItemType edit screen",
						"ItemType not edited", "Fail" });
				Assert.assertEquals(alertMessage, constants.edit_itemtype_successmsg);
			}
		}
		catch (Exception e) {
	...
	public static Object[][] createData() {
		return GeneralActions.getData("EditItemType");
	}
