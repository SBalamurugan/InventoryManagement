...
public class LocationListGlobalFilterFlow extends GeneralActions {
	WebDriver driver;
	...
	/*
	 * LocationFilte Global Filter
	 */
	@Test(priority = 2, dataProvider = "filterData")
	public void locationListGlobalFilterFlow(String sValue) throws Exception {
		try {
			...
			if(commonElements.filtetResultValidation(sValue)){
				TestNGResults.put("28", new Object[] { "LocationList Global Filter",
						"LocationList Global Filter successful", "Pass" });
			} else {
				TestNGResults.put("28", new Object[] { "LocationList Global Filter",
						"LocationList Global Filter Failed", "Fail" });
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static Object[][] filterData() {
		return GeneralActions.getData("LocationListGlobalFilter");
	}
}
