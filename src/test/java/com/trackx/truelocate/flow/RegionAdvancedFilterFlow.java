...
public class RegionAdvancedFilterFlow extends GeneralActions{
	WebDriver driver;
	...
	public void regionAdvancedFilterFlow(String sFiltercode,String sFiltername )throws Exception {
		try {
			imregionelements.menuClick();
			ReusableActions.takeSnapshot(driver, className);
			commonElements.clickAdvancedFilterOpen(driver);
			ReusableActions.takeSnapshot(driver, className);
			imregionelements.enterAdvancedFilterInfo(sFiltercode, sFiltername);
			ReusableActions.takeSnapshot(driver, className);
			commonElements.clickAdvancedSearch(driver);
			Thread.sleep(1000);
			ReusableActions.takeSnapshot(driver, className);
			if(commonElements.filtetResultValidation(sFiltercode)){
				TestNGResults.put("25", new Object[] { "Region Advanced Filter",
						"Region Advance Filter successful", "Pass" });
			} else {
				TestNGResults.put("25", new Object[] { "Region Advance Filter",
						"Region Advance Filter", "Fail" });
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static Object[][] filterData() {
		return GeneralActions.getData("RegionAdvanceFilter");
	}
}
