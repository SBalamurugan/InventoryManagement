...
public class IMRegionDeleteFlow extends GeneralActions {
	WebDriver driver;
	...
	public void regionDeleteFlow(String sValue)throws Exception {
		try {
			...
			if (alertMessage.equalsIgnoreCase(constants.delete_region_successmsg)) {
				TestNGResults.put("17", new Object[] { "Region delete screen",
						"Region delete successfully", "Pass" });	
				Assert.assertEquals(alertMessage, constants.delete_region_successmsg);
			} else {
				TestNGResults.put("17", new Object[] { "Region delete screen",
						"Region not deleted", "Fail" });
				Assert.assertEquals(alertMessage, constants.delete_region_successmsg);
			}
		}
		catch (Exception e) {
	...
	public static Object[][] createData() {
		return GeneralActions.getData("DeleteRegion");
	}
